    ...
    <el-card shadow="hover" class="ri" id="diChart">
        ...
        <div class="di-wrap ub ub-ver">
            <div class="ub ub-f1">
                ...
                <a :href="'#/index/pngDetail?deviceName=' + item.deviceName" class="di-item ub ub-ver ub-pc" style="    width: 50%;">
                    <p style="text-align: center;" class="f20 c-tit ut-s">{{item && item.equipmentName || '　'}}</p>
                    ...
                    <div class="ub ub-ac ub-pc f19"  :class="[ score.isUp ? 'c-gre' : 'c-red']">
                        <div class="c-3">{{score && score.ss && score.ss !== 'NaN' ? score.ss.toFixed(1) : '0'}}</div>
                        <div class="ml15">{{score && score.percent && score.percent != 'NaN' ? score.percent : '0.00'}}%</div>
                    </div>
                </a>
            ...
            <div class="ub f14 fb pt15 pb5">
                <div class="uw0 ub-f1 tx-c">温度: <span class="c-9">{{score.tem && score.tem.toFixed(2)}}°</span></div>
                <div class="uw0 ub-f1 tx-c">电量: <span class="c-9">{{score.ele && score.ele.toFixed(2)}}</span></div>
                <div class="uw0 ub-f1 tx-c">信号: <span class="c-9">{{score.sign && score.sign.toFixed(2)}}</span></div>
            </div>
...
export default {
    ...
    methods:{
        ...
        refreshScore(){
            ...
            rs = this.item.realTimeScore && this.item.realTimeScore !== 'NaN' ? this.item.realTimeScore : 0,
            ss = this.item.startScore && this.item.startScore !== 'NaN' ? this.item.startScore : 0,
            tem = this.item.temperature && this.item.temperature !== 'NaN' ? this.item.temperature : 0;
            ele = this.item.battery && this.item.battery !== 'NaN' ? this.item.battery : 0;
            sign = this.item.rssi && this.item.rssi !== 'NaN' ? this.item.rssi : 0;

            this.score = {
                rs: rs,
                ss: ss,
                percent: (((rs - ss)/ss) * 100).toFixed(2),
                isUp: rs - ss >= 0 ? true : false,
                tem: tem,
                ele: ele,
                sign: sign
