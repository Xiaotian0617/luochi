...
export default {
  ...
  data() {
    return {
      dpr:
        document.getElementsByTagName("html")[0].getAttribute("data-dpr") || 1,
      chartInitOptions: {
        // renderer: 'svg'
      }
    };
  ...
  computed: {
    chartOptions() {
      ...
      let rS = this.item.data.realTimeScore,
        sS = this.item.data.startScore;

      let color = rS - sS >= 0 ? "#14c802" : "#dc0000";

      let xData = [],
        yData = [],
        datas = this.item.data.history;
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: function(param) {
            return (
              "时间:" + param[0].name + "<br/>百分比:" + param[0].value + "%"
            );
          }
        },
        series: [
